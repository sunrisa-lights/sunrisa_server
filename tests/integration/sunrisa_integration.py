...
def test_send_recipe():
    ...
    sio.sleep(1)
    get_recipe_sql = "SELECT recipe_id, recipe_name, power_level, red_level, blue_level, num_hours FROM recipes WHERE recipe_id={}".format(recipe_dict['recipe']['recipe_id'])
    with conn.cursor() as cursor:
...
def test_send_shelf(rack, recipe):
    ...
    sio.sleep(1)
    get_shelf_sql = "SELECT shelf_id, rack_id, recipe_id FROM shelves WHERE shelf_id={}".format(shelf_dict['shelf']['shelf_id'])
    with conn.cursor() as cursor:
