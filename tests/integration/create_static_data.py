...
def create_rooms(sio, num_rooms: int) -> List[int]:
    ...
    for i in range(num_rooms):
        room_dict = {
            "room": {"room_id": i + 1, "is_on": False, "is_veg_room": True, "brightness": 0}
        }
...
def create_racks(sio, num_racks_in_room: int, room_ids: List[int]) -> List[Tuple[int, int]]:
...
def create_shelves(sio, num_shelves_in_rack: int, rack_ids_room_ids: List[Tuple[int, int]]) -> None:
...
def create_static_data(sio):
    ...
    rack_ids_room_ids: List[Tuple[int, int]] = create_racks(sio, num_racks_in_room, room_ids)
