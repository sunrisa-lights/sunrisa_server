...
def init_event_listeners(app_config, socketio):
    ...
    def modify_grow(message) -> None:
        try:
            ...
            if grow_phase_edits_needed:
                ...
                app_config.db.delete_grow_phases_from_grow(
                    db_conn, grow.grow_id
                )
            ...
            if recipe_phase_edits_needed:
                ...
                if grow.is_new_recipe:
                    ...
                    app_config.db.delete_recipe_phases(
                        db_conn, recipe_phases_removed
                    )
                    app_config.db.update_recipe_phases(
                        db_conn, recipe_phases_modified
                    )
                    app_config.db.write_recipe_phases(
                        db_conn, recipe_phases_added
                    )
                else:
                    ...
                    for recipe_phase in new_recipe_phases:
                        if not new_recipe.recipe_id:
                            raise ValueError(
                                "No recipe ID in new_recipe_phases"
                            )
                        else:
                    ...
                    app_config.db.update_grow_recipe(
                        db_conn, grow.grow_id, new_recipe.recipe_id
                    )
            ...
            if grow_phase_edits_needed:
                ...
                app_config.db.write_grow_phases(db_conn, new_grow_phases)
            ...
            if recipe_name != recipe.recipe_name and not was_new_recipe_created:
                ...
                app_config.db.update_recipe_name(db_conn, recipe)
    ...
    def read_grow_with_phases(message) -> None:
        try:
            ...
            grow_phases: List[GrowPhase] = app_config.db.read_grow_phases(
                grow_id
            )
            if not grow_phases:
                send_message_to_namespace_if_specified(
    ...
    def search_recipes(message) -> None:
        try:
            ...
            matching_recipes: List[
                Recipe
            ] = app_config.db.read_recipes_with_name(search_name)
            recipe_ids: List[int] = []
    ...
    def harvest_grow(message) -> None:
        try:
            ...
            app_config.db.end_grow_phase(
                db_conn, current_grow_phase, harvest_datetime
            )
