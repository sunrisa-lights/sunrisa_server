...
def validate_start_grows_for_shelves(app_config: AppConfig, message) -> Tuple[bool, str]:
...
def parse_shelves_from_message(message) -> List[Shelf]:
    ...
    shelves: List[Shelf] = [Shelf.from_json(shelf_json) for shelf_json in shelf_json_list]
    return shelves

# Returns a list of all shelves that are currently in use that are in the planned_shelf_grows list
def validate_all_shelves_free(app_config: AppConfig, planned_shelves: List[Shelf]) -> List[ShelfGrow]:
    ...
    current_shelves_dict: Dict[ShelfGrow, bool] = { Shelf(sg.shelf_id, sg.room_id, sg.rack_id): True for sg in all_current_used_shelf_grows}
